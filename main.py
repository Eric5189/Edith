...
class DrawSomeInfo:
    ...
    def generateLabelArea(self, text, x, y, w, h, frame):
        ...
        res = cv2.addWeighted(sub_img, 0.5, green_rect, 0.5, 1.0)
        res = self.cv2AddText(res, text, (10, 10), textColor=(255, 255, 255), textSize=30)
                        ...
                        if (self.last_finger_arc_degree['Left'] >= 360) and (
                            ...
                            if self.last_detect_res['detection']:
                                ...
                                frame[y:y + h, x:x + w] = self.generateLabelArea(
                                    '{label_zh}'.format(label_zh=self.last_detect_res['detection'][1]), x, y, w, h,
                                    frame)
                            ...
                            if self.hand_mode != 'double':
                                ...
                                # 传给缩略图
                                raw_img = frame_copy[
                                          self.last_finger_cord_y['Right']:self.last_finger_cord_y['Left'],
                                          self.last_finger_cord_x['Right']:self.last_finger_cord_x['Left'], ]
                                frame = self.generateThumb(raw_img, frame)
    ...
    def recognize(self):
        ...
        with self.mp_hands.Hands(min_detection_confidence=0.7,
            ...
            while cap.isOpened():
                ...
                fpsTime = cTime
                self.image = drawInfo.cv2AddText(self.image, "FPS: " + str(int(fps_text)), (10, 20),
                                                 textColor=(0, 255, 0), textSize=40)
                self.image = drawInfo.cv2AddText(self.image, "Palm num: " + str(hand_num), (10, 60),
                                                 textColor=(0, 255, 0), textSize=40)
                self.image = drawInfo.cv2AddText(self.image, "Pattern: " + str(drawInfo.hand_mode), (10, 100),
                                                 textColor=(0, 255, 0), textSize=40)
